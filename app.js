 ...
 * FIXED: merge function, fabric canvas overlay z-index/positioning, redaction, signatures
 */
...
/* ══════════════════════════════════════════════════
   FABRIC.JS SETUP
   FIX: canvas must sit exactly on top of pdfCanvas
══════════════════════════════════════════════════ */
function initFabricCanvas() {
  ...
  // Ensure the wrapper exists for proper stacking
...
function syncFabricSize() {
  ...
  // The fabric canvas upper-canvas must overlay pdf-canvas exactly.
  // We do this by putting both in the same positioned wrapper.
  const wrapper = document.getElementById('canvas-wrapper');
  if (wrapper) {
...
async function onDocumentLoaded() {
  ...
  // Init fabric AFTER canvas is visible
  initFabricCanvas();
...
function deactivateTools() {
  ...
  if (fc) {
    ...
    fc.selection = true;
    // Remove all mouse event listeners from fabric
    fc.off('mouse:down');
    fc.off('mouse:move');
    fc.off('mouse:up');
  }

  // Hide fabric overlay interactivity but keep visible for placed objects
  const hasObjects = fc && fc.getObjects().length > 0;
  if (!hasObjects) {
    DOM.fabricCanvasEl.parentElement.classList.remove('active');
  }
...
/* ══════════════════════════════════════════════════
   REDACT TOOL
   FIX: fabric canvas must be on top and active
══════════════════════════════════════════════════ */
function activateRedactTool() {
  const fc = State.fabricCanvas;
  if (!fc) return;

  // Make the canvas wrapper interactive
  const wrapper = document.getElementById('canvas-wrapper');
...
/* ══════════════════════════════════════════════════
   REORDER PAGES MODAL
   FIX: drag event listeners attached once to grid, not per item
══════════════════════════════════════════════════ */
async function openReorderModal() {
  ...
  // Bind drag events ONCE on the grid container (event delegation)
  bindDragEventsOnGrid();

  hideLoading();
  openModal('reorder-modal');
}

function bindDragEventsOnGrid() {
  ...
  DOM.reorderGrid = newGrid; // update ref
  ...
  newGrid.addEventListener('dragover', e => {
    ...
    if (target && target !== dragSrc) {
      newGrid.querySelectorAll('.reorder-item').forEach(el => el.classList.remove('drag-over'));
  ...
  newGrid.addEventListener('drop', e => {
    ...
    const items = [...newGrid.querySelectorAll('.reorder-item')];
    const srcIdx = items.indexOf(dragSrc);
    const tgtIdx = items.indexOf(target);

    if (srcIdx < tgtIdx) {
      newGrid.insertBefore(dragSrc, target.nextSibling);
    } else {
      newGrid.insertBefore(dragSrc, target);
    }

    newGrid.querySelectorAll('.reorder-item').forEach(el => el.classList.remove('drag-over', 'dragging'));
    dragSrc = null;
  });

  newGrid.addEventListener('dragend', () => {
    newGrid.querySelectorAll('.reorder-item').forEach(el => el.classList.remove('dragging', 'drag-over'));
    dragSrc = null;
  });
}

async function applyReorder() {
  const grid = document.getElementById('reorder-modal').querySelector('.reorder-grid');
...
/* ══════════════════════════════════════════════════
   MERGE PDFs
   FIX: don't clear mergeFiles before using them in toast
══════════════════════════════════════════════════ */
...
async function doMerge() {
  ...
  // Capture count and copy BEFORE clearing state
  const fileCount = State.mergeFiles.length;
  const filesToMerge = [...State.mergeFiles];

  showLoading(`Merging ${fileCount} PDFs…`);
  try {
    const mergedDoc = await PdfUtils.mergePdfs(filesToMerge);
    State.doc     = mergedDoc;
    State.docType = 'pdf';
    State.currentPage = 1;

    // Clear merge UI AFTER successful merge
    State.mergeFiles = [];
    DOM.mergeQueue.classList.add('hidden');
    DOM.mergeFileList.innerHTML = '';
    DOM.fileInput.accept   = '.pdf,.tiff,.tif';
    ...
    await onDocumentLoaded();
    toast(`Merged ${fileCount} PDFs into 1 document.`, 'success');
  } catch (e) {
...
async function applyRedactions() {
  ...
  try {
    ...
    // Re-render page and rebuild thumbnails
...
async function handlePlaceSignature() {
  ...
  // Activate the canvas wrapper so it's interactive
  const wrapper = document.getElementById('canvas-wrapper');
  if (wrapper) wrapper.classList.add('active');
...
function bindEvents() {
  ...
  /* ─ Apply Reorder: delegate to live DOM element ─ */
  ...
  /* ─ Modal Close (FIX: handle both modal-close class and data-modal attr) ─ */
